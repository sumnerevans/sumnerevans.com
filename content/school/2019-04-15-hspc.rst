...
lists each of the problems' names and a short description of the problem:
...
To solve the actual problem, you can just to keep applying the above function on
...
days because `\prod d_i` is guaranteed to be a multiple of `\text{LCM}(d_1, d_2,
d_3)` (proof of this is left to the reader). We call this product the *period*.
