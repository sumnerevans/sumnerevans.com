...
#set document(
  ...
  date: datetime(year: 2019, month: 4, day: 15),
  extra: (
    categories: ("School",),
...
#link("https://icpc.global/")[International Collegiate Programming Contest (ICPC)]. The
...
#link("https://en.wikipedia.org/wiki/Minimum_spanning_tree")[Minimum Spanning Tree]
problem.
...
== Competition Highlights <posts-school-2019-hspc-competition-highlights>
...
== The Problems <posts-school-2019-hspc-the-problems>
...
=== H - Hackaholics <posts-school-2019-hspc-h-hackaholics>
...
=== B - Number Reduction <posts-school-2019-hspc-b-number-reduction>
...
Conjecture at
#link("https://en.wikipedia.org/wiki/Collatz_conjecture")[this Wikipedia article].
...
=== F - Ski Traffic <posts-school-2019-hspc-f-ski-traffic>
...
=== J - Rent Division <posts-school-2019-hspc-j-rent-division>
...
roommates collectively must pay (R + \\sum\_i{e\_i}) dollars worth of expenses,
where (R) is the rent for the month, and (e\_i) is the amount that person
(i) has spent on non-rent expenses. Thus, each roommate must pay a total of

\[ S = \\frac{R + \\sum\_i{e\_i}}{N} \]

worth of expenses that month. If any (e\_i > S) (meaning that the roommate has
paid more than his fair share of the expenses) then you need to output "not
possible". Otherwise, roommate (i) must pay (S - e\_i) worth of the rent to
...
=== D - Darts for Programmers <posts-school-2019-hspc-d-darts-for-programmers>
...
=== A - Alliteration <posts-school-2019-hspc-a-alliteration>
...
+ Iterate through each of the words on the line (using your language's string
  splitting function is advisable for this).
+ Use of a dictionary, or similar data structure, to count how many times each
  letter appears at the start of a word.
+ Output the letter that appears at the start of the most words on that line.
...
=== C - Classrooms an Calculators <posts-school-2019-hspc-c-classrooms-an-calculators>
...
repeat (it's the least-common multiple (LCM) of (d\_1, d\_2, d\_3)), but it is
actually not necessary to compute the LCM for this problem. Instead, you can
just observe that the pattern is guaranteed to repeat at least every (\\prod d\_i
= d\_1 \\times d\_2 \\times d\_3) days because (\\prod d\_i) is guaranteed to be a
multiple of (\\text{LCM}(d\_1, d\_2, d\_3)) (proof of this is left to the reader).
We call this product the _period_.

You then need to extend the pattern for the first (\\prod d\_i) days forward
...
\\underbrace{\\prod d\_i}_{2} + \\underbrace{A\[\\underbrace{(n - 1) mod |A|}\_{3}\]}\_4
\]

where (A) is an array of the day numbers which the friends can play in the
first (\\prod d\_i) days, and (|A|) denotes the length of (A). One way to
...
=== I - Sudoku Verify <posts-school-2019-hspc-i-sudoku-verify>
...
=== E - Tree Skiing <posts-school-2019-hspc-e-tree-skiing>
...
=== G - Misty <posts-school-2019-hspc-g-misty>
...
#link("https://en.wikipedia.org/wiki/Graph_theory")[graph theory] (a branch of computer
science) called the
#link("https://en.wikipedia.org/wiki/Minimum_spanning_tree")[_minimum spanning tree (MST)_ problem].
...
+ Ensures there is a path from every node (house) to every other node.
+ Does that at the minimal _cost_, that is, sum of path distances.

This subgraph will be a
#link("https://en.wikipedia.org/wiki/Spanning_tree")[_spanning tree_] (proof left for
readers enjoyment).

This problem can be solved using a
#link("https://en.wikipedia.org/wiki/Greedy_algorithm")[_greedy_ algorithm] which means
that you do not have to do any global optimization. The
#link("https://en.wikipedia.org/wiki/Minimum_spanning_tree")[Wikipedia page on the MST problem]
...
#link("https://en.wikipedia.org/wiki/Kruskal%27s_algorithm")[_Kruskal's Algorithm_]):

+ Sort all of the edges by length.
+ Iterate until the entire graph has been connected. On each iteration,

  + Get the next shortest edge by length.
  + If adding the edge would create a cycle in the tree, then ignore it.
  + If it would not create a cycle, add it to the tree.
...
#link("https://en.wikipedia.org/wiki/Disjoint-set_data_structure")[disjoint-set data structure]
...
in the competition and I had been working on this problem for nearly a year. I
chose the algorithm and the name of the problem at the same time because if
