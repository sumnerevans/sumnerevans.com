...
#link("https://sumnerevans.com./2021-hspc")[2021], and #link("https://sumnerevans.com./2022-hspc")[2022] competitions
...
the problems and helped #link("https://ezrichards.github.io/")[Ethan Richards] with
...
== The Problems <posts-school-2023-hspc-the-problems>
...
=== Above Sea Level <posts-school-2023-hspc-above-sea-level>
...
=== Driving Dilemma <posts-school-2023-hspc-driving-dilemma>
...
=== MIR Cipher <posts-school-2023-hspc-mir-cipher>
...
=== Sarah's Sandwich Shop <posts-school-2023-hspc-sarahs-sandwich-shop>
...
=== Mines Football <posts-school-2023-hspc-mines-football>
...
=== Cookie Monster Concussion <posts-school-2023-hspc-cookie-monster-concussion>
...
=== Compass Rose <posts-school-2023-hspc-compass-rose>
...
=== Computer Imaging <posts-school-2023-hspc-computer-imaging>
...
- For each computer we need to image, we determine which flash drive it should
  ...
  #image("./images/2023-computer-imaging-diagram.png")
...
flash drive takes (t\_i) seconds to image a computer, then the total number of
computers that can be imaged in (T) seconds is

\[ \\sum\_{i=1}^M \\left\\lfloor \\frac{T}{t\_i} \\right\\rfloor. \]

Note that there are more efficient solutions, which are required for the
#link(<posts-school-2023-hspc-planetary-computer-imaging>)[more difficult form of this problem].

=== Fixing Figures <posts-school-2023-hspc-fixing-figures>
...
=== Paper Pile Pandemonium <posts-school-2023-hspc-paper-pile-pandemonium>
...
=== Noise Reduction <posts-school-2023-hspc-noise-reduction>
...
=== Telescope Targeting <posts-school-2023-hspc-telescope-targeting>
...
=== Wikipedia Black Hole <posts-school-2023-hspc-wikipedia-black-hole>
...
=== Unit Rescue <posts-school-2023-hspc-unit-rescue>
...
=== Planetary Computer Imaging <posts-school-2023-hspc-planetary-computer-imaging>
...
This is a harder version of the #link(<posts-school-2023-hspc-computer-imaging>)[Computer Imaging] problem
...
only (\\log\_2(2 \\cdot 10^{11}) \\approx 38) steps!
...
=== Basketball Modelling <posts-school-2023-hspc-basketball-modelling>
...
Let (f(n, m\_2, m\_3)) be the expected number of points the team scores over
(n) possessions given that they begin with an (m\_2) percent chance of making
a 2-pointer, and an (m\_3) percent chance of making a 3-pointer. We define the
...
For the base case, (f(0, m\_2, m\_3) = 0) for all (m\_2) and (m\_3), because
there are no more possessions to score points.

For the recursive case, let (C\_2) and (C\_3) be the confidence adjustments.
There are five cases to consider.

If the team attempts and makes a 2-pointer, the expected number of points
scored is (2 + f(n - 1, \\min(m\_2 + C\_2, 100), m\_3)). The team has scored 2
...
scored is (0 + f(n - 1, \\max(m\_2 - C\_2, 0), m\_3)). The team did not score
...
probability of attempting a 2-pointer and making it is (\\frac {A\_2}{100}
\\cdot \\frac{m\_2} {100}), for example.
...
=== Pegs <posts-school-2023-hspc-pegs>
...
#image("./images/2023-pegs-move.png", alt: "The valid moves of the peg game")
...
+ *Representing the state of the board in a comparable way.*
  ...
  + *Bitsets*: there are only 15 pegs, so you can fit the entire board state
    in a 32-bit integer (or even a 16-bit one).
  + *Tuples*: if your language has tuples (say, Python), you can use tuples
    of booleans.
  + *Custom class with hash function*: you may have to resort to this in
    languages such as Java.

+ *Finding adjacencies to a given hole.*
...
=== Galactic Reconstruction <posts-school-2023-hspc-galactic-reconstruction>
...
`cluster`: ( \\left\[({1},w\_1),({2},w\_2),({3},3),\\ldots\\right\] )
...
+ add the colony to the larger cluster
+ update the lookup table for each of the moved colonies
...
(\\log\_2(n)) moves before there is only a single cluster.
