...
> her to buy \(A_i\) apples, but she will buy either 1 or \(N\) apples on each
...
> will arrive sequentially and the \(i^{\text{th}}\) customer will order \(A_i\)
...
> [!NOTE] Problem Summary
>
> Kelly has \(M\) hours to complete some tasks. Task \(i\) takes \(T_i\) hours
> and will increase his grade by \(G_i\). Kelly opts to complete the shortest
...
> [!NOTE] Problem Summary
>
> Blake has \(S_i\) shirts of the \(i^{\text{th}}\). He wants to avoid wearing
