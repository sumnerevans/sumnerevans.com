...
#set document(
  ...
  description: "Understanding Matrix cryptography necessitates understanding of some basic\ncryptography primitives. This article intends to explain those prerequisites\nin simple terms.\n",
  date: datetime(year: 2024, month: 8, day: 9),
  extra: (
    categories: ("Matrix", "Programming"),
    tags: ("Cryptography", "E2EE", "Encryption", "Symmetric Encryption", "Asymmetric Encryption", "RSA", "Elliptic Curve Cryptography", "Diffie-Hellman", "Hashes", "HMAC", "HKDF"),
    math: true,
  ),
)
#show: dir-template
...
== Encryption: Symmetric vs Asymmetric <posts-matrix-cryptography-prerequisites-encryption-symmetric-vs-asymmetric>

There are two main categories of encryption schemes:
#link("https://en.wikipedia.org/wiki/Symmetric-key_algorithm")[*symmetric*] and
#link("https://en.wikipedia.org/wiki/Public-key_cryptography")[*asymmetric*].
...
+ *It's slow!* Many systems end up using asymmetric encryption to exchange
  and agree upon a symmetric key, and then use the symmetric key for
  communication.
+ *Classical asymmetric cryptosystems are not quantum-resistant.* All modes
...
- #link("https://en.wikipedia.org/wiki/RSA_(cryptosystem)")[*RSA (Rivest–Shamir–Adleman)*]
  systems base their complexity on the difficulty of factoring prime numbers.
- #link("https://en.wikipedia.org/wiki/Elliptic-curve_cryptography")[*Elliptic Curve*]
  ...
  infeasible" #link("https://en.wikipedia.org/wiki/Elliptic-curve_cryptography#Application_to_cryptography")[^1].
...
== Asymmetric Signatures <posts-matrix-cryptography-prerequisites-asymmetric-signatures>
...
== Hashes and HMAC <posts-matrix-cryptography-prerequisites-hashes-and-hmac>

A
#link("https://en.wikipedia.org/wiki/Cryptographic_hash_function")[cryptographic hash function]
...
accomplish this, we use #link("https://en.wikipedia.org/wiki/HMAC")[HMAC] (how the key
...
== Key-Derivation Functions (HKDF) <posts-matrix-cryptography-prerequisites-key-derivation-functions-hkdf>
...
we can use a
#link("https://en.wikipedia.org/wiki/Key_derivation_function")[Key-Derivation Function (KDF)].
The key-derivation function variant that Matrix uses is
#link("https://en.wikipedia.org/wiki/HKDF")[HKDF] which uses HMAC for the key
derivation process.
...
== Diffie-Hellman Key Exchanges <posts-matrix-cryptography-prerequisites-diffie-hellman-key-exchanges>
...
#link("https://en.wikipedia.org/wiki/Diffie%E2%80%93Hellman_key_exchange")[Diffie-Hellman (DH) Key Exchange]
...
#link("https://en.wikipedia.org/wiki/Elliptic-curve_Diffie%E2%80%93Hellman")[Elliptic-Curve Diffie-Hellman (ECDH)]
...
For example, if we have (A\_{private}) we need (B\_{public}) and if we have
(B\_{private}) we need (A\_{public}).
