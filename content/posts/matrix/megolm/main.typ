...
#set document(
  ...
  description: "In Matrix, message security is provided by Megolm. In this article, I\ndescribe Megolm, and explain what security guarantees it provides and how it\nprovides them.\n",
  date: datetime(year: 2024, month: 8, day: 9),
  extra: (
    categories: ("Matrix", "Programming"),
    tags: ("Cryptography", "E2EE", "Megolm", "Encryption", "Integrity", "Verification", "Ratchet"),
...
#link("https://gitlab.matrix.org/matrix-org/olm/-/blob/master/docs/megolm.md")[README].
Its usage within Matrix is described in
#link("https://spec.matrix.org/v1.11/client-server-api/#mmegolmv1aes-sha2")[Section 10.12.4.3 `m.megolm.v1.aes-sha2`]
of the Matrix Spec.
...
I will start by orienting us in section #link(<posts-matrix-megolm-1-overview>)[1. Overview] and then
discuss the three main security features that Megolm provides in sections
#link(<posts-matrix-megolm-2-message-encryption>)[2. Message Encryption],
#link(<posts-matrix-megolm-3-message-integrity>)[3. Message Integrity], and
#link(<posts-matrix-megolm-4-origin-verification>)[4. Origin Verification]. I will then briefly discuss
how Megolm sessions are shared between devices in section
#link(<posts-matrix-megolm-5-sharing-the-megolm-session>)[5. Sharing the Megolm Session] and then in
section
#link(<posts-matrix-megolm-6-what-is-a-megolm-ratchet-anyway>)[6. What is a Megolm Ratchet Anyway?] I
describe the ratchet mechanism at a high level. I make a few concluding remarks
in section #link(<posts-matrix-megolm-7-conclusion>)[7. Conclusion].

== 1. Overview <posts-matrix-megolm-1-overview>
...
#link(<posts-matrix-megolm-5-sharing-the-megolm-session>)[5. Sharing the Megolm Session].
...
== 2. Message Encryption <posts-matrix-megolm-2-message-encryption>
...
Matrix uses AES-256-CBC\[^2\] which requires a 32-byte key and a 16-byte IV, so
the natural question is: where do we get the key and IV from?

\[^2\]: What "CBC" means is not particularly relevant but included for
completeness' sake.
...
#link(<posts-matrix-megolm-6-what-is-a-megolm-ratchet-anyway>)[6. What is a Megolm Ratchet Anyway?], but
...
== 3. Message Integrity <posts-matrix-megolm-3-message-integrity>
...
== 4. Origin Verification <posts-matrix-megolm-4-origin-verification>
...
== 5. Sharing the Megolm Session <posts-matrix-megolm-5-sharing-the-megolm-session>
...
+ *Olm-encrypted `m.room_key` to-device events.* These are used to send the
  Megolm session to all of the devices in the room when the user sends their
  first message with a specific Megolm session.
+ *Olm-encrypted `m.forwarded_room_key` to-device events.* These are used to
...
+ *Key Backup.* This is a mechanism where the keys are encrypted with a
...
#link("https://cfp.matrix.org/matrixconf2024/talk/TD3KDQ/")[upcoming talk at the Matrix Conference]
...
== 6. What is a Megolm Ratchet Anyway? <posts-matrix-megolm-6-what-is-a-megolm-ratchet-anyway>
...
#link("https://gitlab.matrix.org/matrix-org/olm/-/blob/master/docs/megolm.md")[Megolm documentation]
...
#table(
  columns: 3,
  table.header([], [*People Counter*], [*Megolm*], ),
...
Recall from section #link(<posts-matrix-megolm-2-message-encryption>)[2. Message Encryption] that the bits
...
== 7. Conclusion <posts-matrix-megolm-7-conclusion>
...
/* TODO twyla-convert: hugo shortcode: figure src=&quot;./images/overview.png&quot; title=&quot;Figure 6: Megolm Overview&quot; */
