...
#set document(
  ...
  description: [
    ...
    #link("https://sumnerevans.com./pelican-on-github-pages")[blog post]. Since then, my hosting
  ...
  date: datetime(year: 2019, month: 8, day: 27),
  extra: (
...
= Setting up Pelican <posts-technology-pelican-on-gitlab-pages-setting-up-pelican>
...
+ Make sure that all of your Pelican source code is committed to a branch in
  your repository.
+ Make sure that Pelican is set up with a Makefile which publishes the site to
  ...
  to your needs
  #link("https://gitlab.com/sumner/sumnerevans.com/blob/master/Makefile")[here].
...
+ If you have not already created a `requirements.txt` file, do so now in the
...
= Set up GitLab CI/CD <posts-technology-pelican-on-gitlab-pages-set-up-gitlab-cicd>
...
= Optional: Set up a Custom Domain <posts-technology-pelican-on-gitlab-pages-optional-set-up-a-custom-domain>
...
+ Edit your domain's DNS CNAME record to point to GitLab (GitLab should give
  you the exact CNAME value that you need to use).
+ Add a DNS TXT record to verify that you are, in fact, the owner of the
  domain.
...
= Conclusion <posts-technology-pelican-on-gitlab-pages-conclusion>
